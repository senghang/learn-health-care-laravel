...
class AdmissionService
{
    ...
    {
        return AdmissionModel::where('clinic_id', currentClinic()->id)
            ->where('code', $code)
    ...
    public function admit(array $data): AdmissionModel
    {
        return DB::transaction(function () use ($data) {
            $clinicId = currentClinic()->id;
            $visit = VisitModel::where('clinic_id', $clinicId)
                ->where('code', $data['visit_code'])
                ->firstOrFail();
            ...
            $existing = AdmissionModel::where('clinic_id', $clinicId)
            ...
            if (!empty($data['bed_id'])) {
                $bed = BedModel::where('clinic_id', $clinicId)->findOrFail($data['bed_id']);
                if (!$bed->isAvailable()) {
            ...
            OutInPatientModel::updateOrCreate(
                ['clinic_id' => $clinicId, 'code' => $encounterCode],
                [
                    'clinic_id'      => $clinicId,
                    'visit_code'     => $visit->code,
                    'visit_type'     => 'IPD',
    ...
    public function transferBed(string $admissionCode, int $newBedId): AdmissionModel
    {
        return DB::transaction(function () use ($admissionCode, $newBedId) {
            $clinicId = currentClinic()->id;
            $admission = AdmissionModel::where('clinic_id', $clinicId)
                ->where('code', $admissionCode)
                ->firstOrFail();
            ...
            $newBed = BedModel::where('clinic_id', $clinicId)->findOrFail($newBedId);
            ...
            // Release old bed
            if ($admission->bed_id) {
                $oldBed = BedModel::where('clinic_id', $clinicId)->find($admission->bed_id);
                $oldBed?->release();
            }
    ...
    {
        $admission = AdmissionModel::where('clinic_id', currentClinic()->id)
            ->where('code', $admissionCode)
            ->firstOrFail();
    ...
    public function addMedication(string $admissionCode, array $data): InpatientMedicationModel
    {
        ...
        $medicine = \App\Models\MedicineModel::where('clinic_id', $clinicId)->findOrFail($data['medicine_id']);
        $qty      = (int) ceil((float) ($data['quantity'] ?? 1));

        return DB::transaction(function () use ($admission, $medicine, $data, $qty, $clinicId) {
            // Deduct stock at order time — acquire row lock in same order as PharmacyService
            $locked = \App\Models\MedicineModel::where('clinic_id', $clinicId)
            ...
            if ($locked->stock < $qty) {
                throw new \RuntimeException(
                    "Insufficient stock for {$locked->name}: need {$qty}, have {$locked->stock}."
                );
            }

            $before = $locked->stock;
            $locked->decrement('stock', $qty);
            $after = $locked->fresh()->stock;
            ...
            InventoryTransactionModel::create([
                ...
                'total_price'   => $locked->price * $qty,
                'note'          => 'IPD order: ' . $admission->code,
            ]);
    ...
    public function discharge(string $admissionCode, array $data): AdmissionModel
    {
        return DB::transaction(function () use ($admissionCode, $data) {
            ...
            if ($admission->bed_id) {
                $bed = BedModel::where('clinic_id', $clinicId)->find($admission->bed_id);
                $bed?->release();
            }

            // 3. Close encounter
            $encounter = OutInPatientModel::where('clinic_id', $clinicId)
                ->where('admission_code', $admissionCode)
                ->first();
            $encounter?->update([
            ...
            TreatmentModel::where('clinic_id', $clinicId)
                ->where('admission_code', $admissionCode)
                ->whereIn('status', ['ordered', 'in_progress'])
                ->update(['status' => 'completed']);

            // 5. Mark active IPD medications as completed
            InpatientMedicationModel::where('clinic_id', $clinicId)
                ->where('admission_code', $admissionCode)
                ->where('status', 'active')
                ->update([
                    'status'   => 'completed',
            ...
            $visit = VisitModel::where('clinic_id', $clinicId)
                ->where('code', $admission->visit_code)
                ->first();
            $visit?->update([
    ...
    public function stats(): array
    {
        ...
        return [
            'active_admissions'   => AdmissionModel::where('clinic_id', $clinicId)->active()->count(),
            'today_admissions'    => AdmissionModel::where('clinic_id', $clinicId)->whereDate('admitted_at', today())->count(),
