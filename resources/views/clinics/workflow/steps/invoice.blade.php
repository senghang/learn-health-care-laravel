@php
    ...
    $latestRx   = \App\Models\PrescriptionModel::where('visit_code', $visit->code)->latest()->first();
    $hasRx      = $latestRx !== null;
...
<div class="inv-banner">
    <div class="inv-banner-left">
        ...
        <div>
            ...
            <div class="inv-banner-meta">
                ...
                <span class="badge-s {{ $visit->visit_type==='IPD'?'b-ipd':'b-opd' }}" style="font-size:9px">{{ $visit->visit_type }}</span>
                <span>{{ df_d($visit->admitted_at) }}</span>
                <span class="inv-pay-badge" style="background:{{ ['HEF'=>'#e0f2fe','NSSF'=>'#f0fdf4','CASH'=>'#fefce8','CARD'=>'#f5f3ff'][$paymentType]??'#f5f5f5' }};color:{{ ['HEF'=>'#0369a1','NSSF'=>'#166534','CASH'=>'#854d0e','CARD'=>'#6d28d9'][$paymentType]??'#444' }}">
    ...
    <div class="inv-banner-right">
        ...
        @if($invoice)
        <div class="inv-status-pill" style="background:{{ $sc['bg'] }};color:{{ $sc['color'] }};border:1px solid {{ $sc['bdr'] }}">
    ...
    <table class="tbl" id="svcTable">
        ...
        <tbody id="svcBody">
            ...
            <tr id="svc-row-{{ $i }}">
                ...
                <td><input name="services[{{ $i }}][category]" class="form-control" id="svc-cat-{{ $i }}" value="{{ $svc->service_category }}"/></td>
                <td><input name="services[{{ $i }}][price]" type="number" class="form-control svc-price" id="svc-price-{{ $i }}" value="{{ $svc->price }}" oninput="recalc()"/></td>
                <td><select name="services[{{ $i }}][paid_status]" class="form-select status-sel" onchange="styleStatus(this)">
            ...
            <tr id="svc-row-0">
                ...
                <td><input name="services[0][category]" class="form-control" id="svc-cat-0" value="Consultation"/></td>
                <td><input name="services[0][price]" type="number" class="form-control svc-price" id="svc-price-0" value="30000" oninput="recalc()"/></td>
                <td><select name="services[0][paid_status]" class="form-select status-sel" onchange="styleStatus(this)">
                    <option value="1">Paid ✓</option><option value="0">Pending</option>
                </select></td>
                <td><button type="button" class="btn btn-sm btn-outline-danger" onclick="this.closest('tr').remove();recalc()">✕</button></td>
            </tr>
            @endforelse
            ...
            <tr id="med-row-{{ $i }}">
                <td style="min-width:200px">
                    <x-form.autocomplete
                        ...
                        :input-name="'med_'.$i"/>
                    <input type="hidden" name="inv_meds[{{ $i }}][medicine_code]" id="med-code-{{ $i }}" value="{{ $med->medicine_code }}"/>
                </td>
                <td style="min-width:70px"><input name="inv_meds[{{ $i }}][qty]" type="number" class="form-control med-qty" value="{{ $med->quantity }}" min="1" oninput="calcRow(this)"/></td>
                <td style="min-width:100px"><input name="inv_meds[{{ $i }}][price]" id="med-unit-{{ $i }}" type="number" class="form-control med-unit" value="{{ $med->price }}" oninput="calcRow(this)"/></td>
                <td><div class="inv-row-total" id="row-total-m{{ $i }}">{{ khr($med->payment) }}</div></td>
                <td><select name="inv_meds[{{ $i }}][paid_status]" class="form-select status-sel" onchange="styleStatus(this)">
                    <option value="1" {{ $paid==='1'?'selected':'' }}>Paid ✓</option>
                    <option value="0" {{ $paid==='0'?'selected':'' }}>Pending</option>
                </select></td>
                <td><button type="button" class="btn btn-sm btn-outline-danger" onclick="this.closest('tr').remove();recalc()">✕</button></td>
...
<div class="inv-total-area">
    <div class="inv-breakdown">
        ...
        <div class="inv-breakdown-row" style="font-size:12px;color:#64748b">
            ...
            <span id="bdComputed" style="font-weight:700">{{ khr($services->sum('price') + $meds->sum('payment')) }}</span>
        </div>
    </div>
    <div class="inv-declared">
        ...
        <div id="totalStatus" class="note note-info mt-2" style="font-size:12px;padding:8px 12px;display:inline-flex">
...
<div class="inv-gate">
    ...
    <div class="d-flex gap-2 flex-wrap align-items-center">
        ...
        <button type="submit" name="_complete" value="1" id="completeBtn"
            ...
            @if(!$canComplete)<i class="bi bi-lock-fill" style="font-size:11px;margin-left:2px"></i>@endif
        </button>
    </div>
</div>
...
.inv-banner-left  { display:flex;align-items:center;gap:14px;flex:1;min-width:0; }
.inv-banner-icon  { font-size:28px;flex-shrink:0; }
.inv-banner-name  { font-size:15px;font-weight:800;color:#fff; }
.inv-banner-meta  { font-size:11px;color:#8aabdc;margin-top:4px;display:flex;align-items:center;gap:8px;flex-wrap:wrap; }
.inv-banner-right { text-align:right;flex-shrink:0; }
.inv-banner-code  { font-size:15px;font-weight:800;color:#fff;font-family:monospace; }
.inv-status-pill  { margin-top:4px;font-size:11px;padding:2px 10px;border-radius:10px;font-weight:700;display:inline-block; }
.inv-pay-badge    { font-size:10px;padding:2px 8px;border-radius:10px;font-weight:700; }
.inv-section-hd {
...
.inv-count-chip {
    background:rgba(255,255,255,.2);padding:1px 8px;border-radius:10px;font-size:10px;
}
.inv-add-btn {
...
.inv-add-btn:hover { background:rgba(255,255,255,.3); }
.inv-subtotal {
    font-size:12px;font-weight:600;text-align:right;padding:4px 8px;margin-bottom:4px;
}
.inv-total-area {
...
.inv-breakdown { flex:1;min-width:180px; }
.inv-breakdown-row {
    display:flex;justify-content:space-between;align-items:center;
    font-size:13px;padding:4px 0;color:#374151;
}
.inv-declared { text-align:right;flex-shrink:0; }
.inv-row-total {
    font-size:13px;font-weight:700;color:#012970;white-space:nowrap;
}
.inv-gate {
...
.inv-gate-checks { display:flex;flex-direction:column;gap:6px; }
.inv-gate-item {
    display:flex;align-items:center;gap:8px;font-size:13px;color:#374151;font-weight:600;
}
.stock-badge {
...
.status-sel option[value="1"] { color:#1D9E75;font-weight:700; }
.status-sel option[value="0"] { color:#b45309; }
</style>
...
function sum(sel) {
    return Array.from(document.querySelectorAll(sel))
        .reduce(function(s,e){ return s + (parseFloat(e.value)||0); }, 0);
}
...
function checkTotal() {
    ...
    if (!status) return;
    if (!declared) {
        status.className = 'note note-info mt-2'; status.innerHTML = '<i class="bi bi-info-circle-fill"></i>&nbsp;Enter declared total';
    } else if (Math.abs(declared - computed) < 1) {
        status.className = 'note note-success mt-2'; status.innerHTML = '<i class="bi bi-check-circle-fill"></i>&nbsp;<strong>ត្រូវគ្នា / Verified ✓</strong>';
    } else {
        status.className = 'note note-danger mt-2'; status.innerHTML = '<i class="bi bi-exclamation-triangle-fill"></i>&nbsp;Mismatch — computed: <strong>'+fmt(computed)+'</strong>';
    }
}
...
function updateStatus(val) {
    ...
    if (icon) { icon.className = 'bi ' + (isPaid ? 'bi-check-circle-fill' : 'bi-circle'); icon.style.color = isPaid ? '#1D9E75' : '#94a3b8'; }
    if (txt)  { txt.textContent = 'Invoice ' + (isPaid ? 'Paid ✓' : 'not paid yet'); }
}
...
function addSvcRow() {
    ...
    tr.innerHTML =
        ...
        + '<button type="button" id="acClear_svc_'+idx+'" style="position:absolute;right:8px;top:50%;transform:translateY(-50%);background:none;border:none;color:#94a3b8;font-size:14px;cursor:pointer;padding:0;display:none;line-height:1;z-index:2" onclick="acClear('svc_'+idx+'')">'
...
function initSvcAC(idx) {
    ...
    function filterSvc(q) {
        ...
        q = q.toLowerCase();
        return svcCatalog.filter(function(c){ return (c.name||'').toLowerCase().indexOf(q) !== -1 || (c.category||'').toLowerCase().indexOf(q) !== -1; }).slice(0, 12);
    }

    function esc(s){ return String(s||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;'); }

    function renderSvcDd(q) {
        filtered = filterSvc(q); activeIdx = -1;
        if (!filtered.length) {
    ...
    window['pickSvc_'+idx] = function(i) {
        var item = filtered[i]; if (!item) return;
        ...
        if (priEl) { priEl.value = item.price || 0; recalc(); }
        elClear.style.display = 'block';
        elDd.style.display = 'none';
    };

    function pickSvc(ri, i) { if (ri === idx) window['pickSvc_'+idx](i); }
    window['pickSvc'] = function(ri, i) { if (ri === idx) window['pickSvc_'+idx](i); };

    elText.addEventListener('input', function() {
        elClear.style.display = this.value ? 'block' : 'none';
        renderSvcDd(this.value.trim());
    });
    elText.addEventListener('focus', function() { renderSvcDd(this.value.trim()); });
    document.addEventListener('click', function(e) { if (!elWrap?.contains(e.target)) elDd.style.display = 'none'; });
...
function autoFillSvc(input, idx) {
    var item = svcCatalog.find(function(c){ return c.name === input.value; });
    if (!item) return;
    var catEl = document.getElementById('svc-cat-'+idx);
    var priEl = document.getElementById('svc-price-'+idx);
    if (catEl) catEl.value = item.category;
    if (priEl) { priEl.value = item.price; recalc(); }
}
...
function initMedAC(idx) {
    ...
    function filterMed(q) {
        ...
        q = q.toLowerCase();
        return medCatalog.filter(function(c){ return (c.name||'').toLowerCase().indexOf(q)!==-1 || (c.strength||'').toLowerCase().indexOf(q)!==-1 || (c.form||'').toLowerCase().indexOf(q)!==-1; }).slice(0,12);
    }
    function esc(s){ return String(s||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;'); }
    ...
    elDd.addEventListener('mousedown', function(e) {
        ...
        e.preventDefault();
        var rows = Array.from(elDd.querySelectorAll('.ac-row'));
        var item = filtered[rows.indexOf(row)]; if (!item) return;
        elText.value = item.name;
        elId.value   = item.id;
        elClear.style.display = 'block';
        elDd.style.display = 'none';
        ...
        if (badge) {
            ...
            if (item.stock <= 0) { badge.textContent='Out of stock'; badge.className='stock-badge stock-out'; }
            else if (item.stock <= item.alert) { badge.textContent='Low: '+item.stock; badge.className='stock-badge stock-low'; }
            else { badge.textContent='\u2713 In stock: '+item.stock; badge.className='stock-badge stock-ok'; }
        }
    ...
    elText.addEventListener('focus', function() { renderMedDd(this.value.trim()); });
    document.addEventListener('click', function(e) { if (!elWrap || !elWrap.contains(e.target)) elDd.style.display = 'none'; });
}

function fillMedRow(sel, idx) {
    var opt = sel.options[sel.selectedIndex];
    if (!opt.value) return;
    var item = medCatalog.find(function(c){ return c.id == opt.value; });
    if (!item) return;
    setVal('med-code-'+idx, item.code);
    setVal('med-name-'+idx, item.name);
    setVal('med-unit-'+idx, item.price);
