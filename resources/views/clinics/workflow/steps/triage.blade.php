...
<div style="height:3px;background:#f0f2ff">
    <div
        style="height:100%;width:{{ round($stepIdx/count($steps)*100) }}%;background:linear-gradient(90deg,#4154f1,#717ff5)">
    </div>
</div>
...
<div class="card-bd">
    ...
    <form id="stepForm" method="POST" action="{{ route('workflow.step.save', [$visit->code, 'triage']) }}">
        ...
        <div class="row g-3">
            <div class="col-6">
                <div class="fld">
                    <label class="flbl"><span class="km">លេខ Encounter</span><span
                            class="en">/ Encounter Code</span></label>
                    <input class="form-control ro" value="{{ $triage->encounter_code ?? 'TR'.$visit->code }}"
                           readonly/>
                </div>
            </div>
            <div class="col-6">
            ...
            <div class="col-6">
                <div class="fld">
                    ...
                    <input type="datetime-local" name="recorded_at" class="form-control"
                           value="{{ old('recorded_at', $triage?->recorded_at) }}"/>
                </div>
            </div>
            <div class="col-6">
                <div class="fld">
                    ...
                    <select name="title" class="form-select">
                        @foreach(['វេជ្ជបណ្ឌិត / Doctor','គិលានុបដ្ឋាយិកា / Nurse','Paramedic'] as $t)
                            <option
                                value="{{ $t }}" {{ old('title', $triage->title ?? '') === $t ? 'selected':'' }}>{{ $t }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="col-6">
                <div class="fld">
                    ...
                    <input type="number" name="weight" class="form-control" placeholder="65"
                           value="{{ old('weight', $triage->weight ?? '') }}"/>
                </div>
            </div>
            <div class="col-12">
                <div class="fld">
                    <label class="flbl"><span class="km">ហេតុការណ៍ចូល</span><span
                            class="en">/ Chief Complaint</span></label>
                    <textarea name="chief_complaint" class="form-control" rows="3"
                              placeholder="ពណ៌នា… / Describe…">{{ old('chief_complaint', $triage->chief_complaint ?? '') }}</textarea>
                </div>
            </div>
        </div>
    </form>
