...
<x-page-header
    ...
    <a href="{{ route('print.prescription', $prescription->code) }}" class="btn btn-outline-secondary btn-sm" target="_blank">
        ...
        <div class="card-emr mb-3">
            <div class="card-hd" style="background:#fdf0f8">
                <div class="card-hd-title">
                    ...
                    <span style="background:#e91e8c22;color:#e91e8c;border:1px solid #e91e8c66;font-size:10px;padding:1px 8px;border-radius:10px;margin-left:6px">
                        {{ count($items) }} items
                    </span>
                </div>
                @if(!$isFullyDone)
                    <label style="display:flex;align-items:center;gap:6px;font-size:11.5px;color:#4154f1;cursor:pointer">
            ...
            <div class="card-bd" style="padding:0">
                <div class="table-responsive">
                    <table class="tbl">
                        <thead>
                            <tr>
                                @if(!$isFullyDone)<th style="width:36px"></th>@endif
                                <th>#</th>
                                <th>Medicine</th>
                        ...
                        @foreach($items as $i => $row)
                            ...
                            <tr style="{{ $done ? 'opacity:.55' : '' }}">
                                ...
                                <td style="text-align:center">
                                    ...
                                    @elseif($done)
                                        <i class="bi bi-check-circle-fill" style="color:#2eca6a;font-size:14px"></i>
                                    @else
                                ...
                                <td>
                                    ...
                                    @if($med->strength)
                                        <code style="font-size:10.5px;background:#eef0fd;color:#4154f1;padding:1px 5px;border-radius:4px">
                                            {{ $med->strength }}
                                        </code>
                                    @endif
                                    @if($med->form)
                                        <span style="font-size:10px;color:#9b59b6;margin-left:4px">{{ $med->form }}</span>
                                    @endif
                                    @if($med->note)
                                        <div style="font-size:10px;color:#aaa;font-style:italic">{{ $med->note }}</div>
                                    @endif
                                ...
                                <td style="text-align:center">
                                    @if($done)
                                        <span style="font-size:11px;background:#e8f8ef;color:#1D9E75;border:1px solid #b7eacf;padding:3px 8px;border-radius:8px;font-weight:700">
                                            ✅ Dispensed
                                        </span>
                                    @elseif($canDispense)
                                        <span style="font-size:11px;background:#fff4ec;color:#ff771d;border:1px solid #ffd0a8;padding:3px 8px;border-radius:8px;font-weight:700">
                                            ⏸ Pending
                                        </span>
                                    @else
                                        <span style="font-size:11px;background:#fde8e8;color:#dc2626;border:1px solid #fca5a5;padding:3px 8px;border-radius:8px;font-weight:700">
                                            ✗ Cannot
                                        </span>
                                    @endif
                                </td>
    ...
    <div class="col-12 col-lg-4">
        ...
        <div class="card-emr mb-3">
            ...
            <div class="card-bd">
                ...
                <div style="margin-top:10px;padding-top:10px;border-top:1px solid #f0f2ff;text-align:center">
                    <span style="font-size:12px;padding:5px 14px;border-radius:10px;font-weight:800;background:{{ $statusBg }};color:{{ $statusColor }}">
                        {{ $statusLabel }}
                    </span>
                </div>
            </div>
        ...
        <div class="card-emr mb-3">
            <div class="card-hd" style="background:#f6f9ff">
                <div class="card-hd-title"><i class="bi bi-person-vcard-fill" style="color:#4154f1"></i> Patient</div>
            ...
            <div class="card-bd">
                <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px">
                    <div style="width:38px;height:38px;border-radius:10px;background:{{ $col }};color:#fff;display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:800;flex-shrink:0">
                        {{ strtoupper(substr($pt->surname,0,1) . substr($pt->name,0,1)) }}
                    </div>
                    <div>
                        <div style="font-weight:800;color:#012970;font-size:14px">{{ $pt->surname }}, {{ $pt->name }}</div>
        ...
        <div class="card-emr" style="position:sticky;top:76px">
            ...
            <div class="card-bd">
                ...
                @if($anyPending > 0)
                    {{-- FULL DISPENSE --}}
                    @if($canFull)
                    <form method="POST" action="{{ route('pharmacy.dispense', $prescription->code) }}" id="fullForm">
                        @csrf
                    ...
                    {{-- PARTIAL DISPENSE --}}
                    <form method="POST" action="{{ route('pharmacy.dispense', $prescription->code) }}" id="partialForm">
                        @csrf
                ...
                {{-- Divider --}}
                <div style="margin-top:16px;padding-top:16px;border-top:1px solid #f0f2ff;font-size:11.5px;color:#aaa">
