            ...
            <div class="card-emr mb-3">
                <div class="card-hd" style="background:#fdf0f8">
                    <div class="card-hd-title">
                        ...
                        {{ __('app.prescription') }}
                        <code
                            style="font-size:12px;color:#e91e8c;background:#fce4f4;padding:2px 8px;border-radius:6px">{{ $prescription->code }}</code>
                    </div>
                    <span
                        style="font-size:11px;color:#aaa">{{ $prescription->prescribed_at?->format('d/m/Y H:i') }}</span>
                </div>
                <div class="card-bd">
                    <div class="row g-3 mb-4">
                        <div class="col-6 col-sm-3">
                            <div
                                style="font-size:10.5px;color:#aaa;font-weight:700;text-transform:uppercase;letter-spacing:.4px">{{ __('app.patient.name.name') }}</div>
                            <div
                                style="font-weight:700;color:#012970;margin-top:3px">{{ $prescription->patient?->surname }}
                                , {{ $prescription->patient?->name }}</div>
                            <div style="font-size:10.5px;color:#aaa">{{ $prescription->patient_code }}</div>
                        </div>
                        <div class="col-6 col-sm-3">
                            <div
                                style="font-size:10.5px;color:#aaa;font-weight:700;text-transform:uppercase;letter-spacing:.4px">{{ __('app.doctor') }}</div>
                            <div
                                style="font-weight:600;color:#555;margin-top:3px">{{ $prescription->prescribed_by ?? '—' }}</div>
                        </div>
                        <div class="col-6 col-sm-3">
                            <div
                                style="font-size:10.5px;color:#aaa;font-weight:700;text-transform:uppercase;letter-spacing:.4px">{{ __('app.visit.code') }}</div>
                        ...
                        <div class="col-6 col-sm-3">
                            <div
                                style="font-size:10.5px;color:#aaa;font-weight:700;text-transform:uppercase;letter-spacing:.4px">{{ __('app.medications_count') }}</div>
                            <div
                                style="font-size:22px;font-weight:800;color:#e91e8c;margin-top:2px">{{ $prescription->medications->count() }}</div>
                        </div>
                    </div>

                    {{-- Medications table --}}
                    <div
                        style="font-size:10.5px;font-weight:800;text-transform:uppercase;letter-spacing:.5px;color:#e91e8c;margin-bottom:10px">
                        {{ __('app.medications') }}
                    </div>
                    <div class="table-responsive">
                        <table class="tbl">
                            ...
                            @forelse($prescription->medications as $i => $med)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @if($med->note)
                                            <div
                                                style="font-size:10.5px;color:#9b59b6;font-style:italic">{{ $med->note }}</div>
                                        @endif
                                    </td>
                                    <td>
                                        @if($med->form)
                                            <span
                                                style="font-size:11px;background:#f0f2ff;color:#4154f1;padding:2px 8px;border-radius:6px">{{ $med->form }}</span>
                                        @endif
                                        @if($med->method)
                                            <span
                                                style="font-size:10.5px;color:#aaa;margin-left:4px">{{ $med->method }}</span>
                                        @endif
                                    </td>
                                    ...
                                    <td>
                                        ...
                                        <span
                                            style="font-weight:800;color:#e91e8c">{{ $total > 0 ? $total : '—' }}</span>
                                        @if($med->unit)
                            ...
                            @empty
                                <tr>
                                    <td colspan="9"
                                        style="text-align:center;color:#bbb;padding:20px">{{ __('app.no_medications') }}</td>
                                </tr>
                            @endforelse
                            </tbody>
                        </table>
            ...
            @if($prescription->patient)
                <div class="card-emr" style="position:sticky;top:76px">
                    <div class="card-hd" style="background:#f6f9ff">
                        <div class="card-hd-title"><i class="bi bi-person-vcard-fill"
                    ...
                    <div class="card-bd">
                        ...
                        <div style="display:flex;align-items:center;gap:12px;margin-bottom:14px">
                            <div
                                style="width:44px;height:44px;border-radius:12px;background:{{ $col }};color:#fff;display:flex;align-items:center;justify-content:center;font-size:16px;font-weight:800;flex-shrink:0">
                                {{ strtoupper(substr($pt->surname,0,1).substr($pt->name,0,1)) }}
                            </div>
                            <div>
                                <div style="font-weight:800;font-size:14px;color:#012970">{{ $pt->surname }}
                                    , {{ $pt->name }}</div>
                        ...
                        <div style="margin-top:12px;padding-top:12px;border-top:1px solid #f0f2ff">
                            <a href="{{ route('patients.show', $pt->code) }}"
