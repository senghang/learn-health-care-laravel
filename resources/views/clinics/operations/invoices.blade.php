                ...
                <div class="row g-2 align-items-end">
                    ...
                    <div class="col-6 col-sm-2">
                        <select name="status" class="form-select">
                            ...
                            <option
                                value="pending" {{ request('status')==='pending'?'selected':'' }}>{{ __('app.pending') }}</option>
                            <option
                                value="partial" {{ request('status')==='partial'?'selected':'' }}>{{ __('app.partial') }}</option>
                            <option
                                value="paid" {{ request('status')==='paid'?'selected':'' }}>{{ __('app.paid') }}</option>
                        </select>
                    </div>
                    <div class="col-6 col-sm-2">
                    ...
                    <div class="col-auto d-flex gap-2">
                        ...
                        @if(request()->hasAny(['search','status','payment_type','date']))
                            <a href="{{ route('invoices.index') }}" class="btn btn-outline-secondary"><i
                ...
                <table class="tbl">
                    ...
                    @forelse($invoices as $inv)
                        ...
                        <tr onclick="window.location='{{ route('invoices.show', $inv->code) }}'" style="cursor:pointer">
                            <td>
                                <div
                                    style="font-size:12px;font-weight:600;color:#012970">{{ $inv->invoice_date?->format('d/m/Y') ?? '—' }}</div>
                            </td>
                            <td><code style="font-size:11px;color:#00bcd4">{{ $inv->code }}</code></td>
                            <td>
                                @if($inv->patient)
                                    <div
                                        style="font-weight:600;color:#012970;font-size:13px">{{ $inv->patient->surname }}
                                        , {{ $inv->patient->name }}</div>
                            ...
                            <td>
                        <span
                            style="font-size:10.5px;background:{{ ($ptColors[$inv->payment_type]??'#aaa') }}22;color:{{ $ptColors[$inv->payment_type]??'#aaa' }};padding:2px 9px;border-radius:10px;font-weight:700;border:1px solid {{ ($ptColors[$inv->payment_type]??'#aaa') }}44">
                            {{ $inv->payment_type }}
                        </span>
                            </td>
                            ...
                            <td>
                        <span
                            style="font-size:10.5px;background:{{ $sc['bg'] }};color:{{ $sc['color'] }};padding:2px 10px;border-radius:10px;font-weight:700;border:1px solid {{ $sc['color'] }}44">
                            {{ $sc['label'] }}
                        </span>
                            </td>
                            <td onclick="event.stopPropagation()">
                                <div style="display:flex;gap:4px">
                                    ...
                                    @if($balance > 0)
                                        <a href="{{ route('invoices.show', $inv->code) }}#payment"
