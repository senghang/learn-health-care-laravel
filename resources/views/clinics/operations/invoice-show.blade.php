...
<div class="col-12 col-lg-8">
    ...
    <div class="card-emr mb-3">
        <div class="card-hd" style="background:#f0fcff">
            <div class="card-hd-title">
                ...
                {{ __('app.invoice') }}
                <code style="font-size:12px;color:#00bcd4;background:#e0f7fa;padding:2px 8px;border-radius:6px">{{ $invoice->code }}</code>
            </div>
        ...
        <div class="card-bd">
            <div class="row g-3 mb-4">
                @foreach([
                    [__('app.patient'),      $invoice->patient?->surname.', '.$invoice->patient?->name, $invoice->patient_code],
                    [__('app.visit'),        $invoice->visit_code ?? '—', null],
                ...
                <div class="col-6 col-sm-3">
                    ...
                    @if($sub)<div style="font-size:10.5px;color:#aaa">{{ $sub }}</div>@endif
                </div>
                @endforeach
            </div>
            ...
            <div class="table-responsive mb-4">
                <table class="tbl">
                    <thead><tr><th>{{ __('app.service') }}</th><th>{{ __('app.category') }}</th><th style="text-align:right">{{ __('app.price') }} KHR</th></tr></thead>
                    <tbody>
            ...
            <div class="table-responsive mb-4">
                <table class="tbl">
                    <thead><tr><th>{{ __('app.medicine') }}</th><th>{{ __('app.qty') }}</th><th style="text-align:right">{{ __('app.price') }} KHR</th></tr></thead>
                    <tbody>
    ...
    <div class="card-emr mb-3">
        <div class="card-hd">
            <div class="card-hd-title"><i class="bi bi-cash-stack" style="color:#2eca6a"></i> {{ __('app.payment_history') }}</div>
        </div>
        <div class="card-bd" style="padding:0">
            <table class="tbl">
                <thead><tr><th>{{ __('app.date') }}</th><th>{{ __('app.method') }}</th><th>{{ __('app.reference') }}</th><th>{{ __('app.collected_by') }}</th><th style="text-align:right">{{ __('app.amount') }} KHR</th></tr></thead>
                <tbody>
...
<div class="col-12 col-lg-4">
    ...
    <div class="card-emr mt-3">
        <div class="card-hd" style="background:#f6f9ff">
            <div class="card-hd-title"><i class="bi bi-person-fill" style="color:#4154f1"></i> {{ __('app.patient') }}</div>
        </div>
        <div class="card-bd">
            ...
            <a href="{{ route('patients.show', $invoice->patient->code) }}" class="btn btn-outline-primary btn-sm btn-w100">
